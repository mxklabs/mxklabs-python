  ...
  def basic_ops_check(exprid, min_ops, max_ops, exp_op_valtype, ops):
    ...
    for op_index, op in zip(range(len(ops)), ops):
      if not isinstance(op, Expr):
        raise RuntimeError(f"'{exprid}' expects operands of type mxklabs.expr.Expr (operand {op_index} has type {type(op)})")

    # Check optypes match.
    if exp_op_valtype is not None:
      for op_index, op in zip(range(len(ops)), ops):
        if op.valtype != exp_op_valtype:
          raise RuntimeError(f"'{exprid}' expects operands of type '{exp_op_valtype.id()}' (operand {op_index} has type '{op.valtype.id()}')")


  @staticmethod
  def basic_sub_valtypes_check(valtypeid, min_sub_valtypes, max_sub_valtypes, exp_sub_valtype, sub_valtypes):
    ...
    if (min_sub_valtypes is not None and \
      ...
      if (min_sub_valtypes != len(sub_valtypes)):
        raise RuntimeError(f"'{valtypeid}' expects exactly {min_sub_valtypes} sub_valtypes{'s' if min_sub_valtypes > 1 else ''} (got {len(sub_valtypes)})")
    else:
      if (min_sub_valtypes is not None and len(sub_valtypes) < min_sub_valtypes):
        raise RuntimeError(f"'{valtypeid}' expects at least {min_sub_valtypes} sub_valtypes{'s' if min_sub_valtypes > 1 else ''} (got {len(sub_valtypes)})")
      if (max_sub_valtypes is not None and len(sub_valtypes) > max_sub_valtypes):
        raise RuntimeError(f"'{valtypeid}' expects at most {max_sub_valtypes} sub_valtypes{'s' if max_sub_valtypes > 1 else ''} (got {len(sub_valtypes)})")
    ...
    for index, sub_valtype in zip(range(len(sub_valtypes)), sub_valtypes):
      if not isinstance(op, Valtype):
        raise RuntimeError(f"'{valtypeid}' expects sub_valtypes of type mxklabs.expr.Valtype (sub_valtypes {index} has type {type(sub_valtype)})")

    # Check sub_valtypes match.
    if exp_sub_valtype is not None:
      for index, sub_valtype in zip(range(len(sub_valtypes)), sub_valtypes):
        if sub_valtype != exp_sub_valtype:
          raise RuntimeError(f"'{valtypeid}' expects sub_valtypes of type '{exp_sub_valtype.id()}' (sub_valtypes {index} has type {sub_valtype.id()})")
