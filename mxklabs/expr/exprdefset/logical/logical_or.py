...
class LogicalOr(LogicalExprDef):
  ...
  def simplify(self, expr):
    ...
    # Remove any false ops.
    filtered_ops = [op for op in expr.ops() if not self._ctx.is_constant(op) or op.value()]
    if len(filtered_ops) < len(expr.ops()):
      ...
      # Multiple, just remove the terms.
        ...
        return self._ctx.expr.logical_or(filtered_ops)
