...
class Test_Expr(unittest.TestCase):
  ...
  def test_hashstr(self):    
    ...
    self.assertEqual(mxk.LogicalAnd(mxk.Variable(type=mxk.Bool(), id="v1"),mxk.Constant(type=mxk.Bool(), user_value=True)).hash_str(), "(logical-and (var v1) (const bool true))")
    ...
    self.assertEqual(mxk.LogicalOr(mxk.Constant(type=mxk.Bool(), user_value=False),mxk.Variable(type=mxk.Bool(), id="v1")).hash_str(), "(logical-or (const bool false) (var v1))")
